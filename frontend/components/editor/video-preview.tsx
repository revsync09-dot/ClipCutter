...
export const VideoPreview = forwardRef<HTMLVideoElement, VideoPreviewProps>(function VideoPreview({ project }, forwardedRef) {
  ...
  const stacked=hasReaction&&(design.layout==='reaction_top'||design.layout==='main_focus'||design.layout==='main_top');
  ...
  const mainClass=!hasReaction?'inset-0 h-full w-full object-contain':design.layout==='main_focus'?'left-0 top-[42%] h-[34%] w-full object-cover':design.layout==='reaction_top'?(socialSafe?'left-0 top-[37.5%] h-[31.7%] w-full object-cover':'left-0 top-[38%] h-[58%] w-full object-cover'):design.layout==='main_top'?'left-0 top-0 h-[44%] w-full object-cover':design.layout==='picture_in_picture'?'inset-0 h-full w-full object-cover':'left-[12%] top-[23%] h-[48%] w-[76%] object-cover shadow-2xl';
  const reactionClass=design.layout==='main_focus'?'left-0 top-0 h-[34%] w-full object-cover':design.layout==='reaction_top'?(socialSafe?'left-0 top-0 h-[35%] w-full object-cover':'left-0 top-0 h-[38%] w-full object-cover'):design.layout==='main_top'?'left-0 top-[44%] h-[56%] w-full object-cover':design.layout==='picture_in_picture'?'right-[4%] top-[4%] h-[28%] w-[34%] rounded-xl border-2 border-white object-cover shadow-2xl':'inset-0 h-full w-full object-cover opacity-30 blur-xl';
  ...
  return <div className="w-full rounded-[30px] border border-ink/[.08] bg-white p-2 shadow-[0_18px_55px_rgba(32,20,46,.10)]">
    ...
    {hasFrame&&stacked&&<img src={projectMediaUrl(project, `/api/projects/${project.id}/frame/media`)} alt="Reaction-Rahmen" draggable={false} className={`pointer-events-none absolute left-0 z-[7] w-full object-contain ${design.layout==='reaction_top'||design.layout==='main_focus'?(socialSafe?'top-0 h-[35%]':design.layout==='main_focus'?'top-0 h-[34%]':'top-0 h-[38%]'):'top-[44%] h-[56%]'}`}/>} 
